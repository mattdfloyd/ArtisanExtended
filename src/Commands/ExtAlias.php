...
class ExtAlias extends Command
{
	...
	public function handle()
	{
		...
		{
			return $this->error('You have to enclose the path in " (double quotes) to make sure that the backslashes are correctly saved.');
		}

		$alias_line = '        ' . '\'' . $alias_name . '\'	=>	' . $alias_path . '::class,';
		...
		else
		{
			...
			$config_file_data_new = substr_replace($config_file_data, $alias_line . "\n", $pos + 68, 0);
		}

		file_put_contents($config_file, $config_file_data_new);

		return $this->info('The following alias has been succesfully added: ' . $alias_path);
