...
class ExtHostUrl extends Command
{
    ...
    if (file_exists($file))
    {
      file_put_contents($file, str_replace("'url' => '" . $old_url . "',","'url' => '" . $this->argument('url') . "',", file_get_contents($file)));

      return $this->info('The URL has been changed successfully to: ' . $this->argument('url'));
    }
	...
	{
		return array(
			array('url', InputArgument::REQUIRED, 'URL of your application'),
		);
	}
